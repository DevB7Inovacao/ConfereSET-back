...
namespace ControlApi.Controllers
{
    ...
    public class ChecklistController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpPost("sync")]
        public async Task<IActionResult> Sync([FromBody] SyncChecklistVariavelRequest req)
        {
            try
            {
        ...
        [AllowAnonymous]
        [HttpGet("variaveis")]
        public async Task<IActionResult> GetVariaveisByChecklist([FromQuery] int empresaId, [FromQuery] int checklistId, [FromQuery] bool onlyActiveLinks = true)
        {
            try
            {
                var result = await _service.GetVariaveisByChecklist(empresaId, checklistId, onlyActiveLinks);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [AllowAnonymous]
        [HttpPost("render/{checklistId}")]
        public async Task<IActionResult> Render([FromRoute] int checklistId, [FromQuery] int empresaId, [FromBody] RenderChecklistRequest req)
        {
            try
            {
                var result = await _service.Render(empresaId, checklistId, req);
                return Ok(result);
            }
            catch (Exception ex)
            {
